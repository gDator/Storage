...
void Console::draw()
{
    ...
    for (int i = 0; i < log.size(); i++)
    {
        ...
        if (strstr(item, "[error]"))          { color = ImVec4(0.8f, 0.3f, 0.3f, 1.0f); has_color = true; }
        else if (strncmp(item, "#", 2) == 0) { color = ImVec4(0.5f, 0.5f, 0.5f, 1.0f); has_color = true; }
        else if (strstr(item, "[warning]")) { color = ImVec4(0.5f, 0.4f, 0.f, 1.0f); has_color = true; }
